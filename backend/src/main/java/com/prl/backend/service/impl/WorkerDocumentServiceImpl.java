...
public class WorkerDocumentServiceImpl implements WorkerDocumentService {
        ...
        public WorkerDocumentResponse upload(UploadWorkerDocumentRequest request, MultipartFile file) {
                ...
                // If the DB rejects it (constraint, validation, etc.) no orphan file is
                // created.
        ...
        public Map<String, Object> getComplianceStatus(Long workerId) {
                ...
                // WORKER can only query their own compliance
                if (currentUser.getRole() == Role.WORKER
                                && !currentUser.getId().equals(workerId)) {
                        throw new AccessDeniedException("Solo puedes consultar tu propio cumplimiento");
                }
                ...
                for (WorkerDocument doc : workerDocs) {
                        ...
                        if (mandatoryDocTypeIds.contains(docTypeId)) {
                                latestByDocType.merge(
                                                docTypeId, doc, (existing,
                                                                newer) -> newer.getUploadedAt().isAfter(
                                                                                existing.getUploadedAt()) ? newer
                                                                                                : existing);
                        }
                }
