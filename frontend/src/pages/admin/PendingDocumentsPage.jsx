...
function fmt(d) { if (!d) return '—'; return new Date(d).toLocaleDateString('es-ES') }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form id="rvf" onSubmit={handleSubmit} className="space-y-4 py-2">
          <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm space-y-1">
            <p><span className="font-medium">Trabajador:</span> {doc.workerName ?? '—'}</p>
            <p><span className="font-medium">Documento:</span> {doc.documentTypeName ?? '—'}</p>
          </div>

          {isApprove ? (
            <p className="text-sm text-muted-foreground">
              ¿Confirmas que el documento es válido y cumple todos los requisitos?
            </p>
          ) : (
...
export default function PendingDocumentsPage() {
  ...
  const load = async () => {
    try { setDocs(await workerDocumentApi.getPending()) }
    catch { toast.error('Error al cargar documentos pendientes') }
  ...
  return (
    <div className="p-6 space-y-6">
              ...
              {docs.length === 0 ? (
                ...
                docs.map((doc) => (
                  <TableRow key={doc.id}>
                    ...
                    <TableCell className="text-muted-foreground">{fmt(doc.uploadedAt ?? doc.createdAt)}</TableCell>
                    <TableCell className="text-muted-foreground">{fmt(doc.expiryDate)}</TableCell>
      ...
      <ReviewDialog
        open={!!reviewDoc}
        onOpenChange={(v) => { if (!v) { setReviewDoc(null); setReviewAction(null) } }}
        doc={reviewDoc}
        action={reviewAction}
        onSuccess={load}
      />
